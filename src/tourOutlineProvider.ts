...
export class TourOutlineProvider implements vscode.WebviewViewProvider {
  ...
  setState(
    ...
    this.state = { activeTour, tourStack, backendLabel, questionDraft, hasSession, currentModel, availableModels, isTourVisible, backendChoice };
    this.render();
  }
  ...
  private getHtml(state: SidebarState): string {
    const { activeTour, tourStack, backendLabel, questionDraft, hasSession, currentModel, availableModels, isTourVisible, backendChoice } = state;
          ...
          <div class="stackList">
            ${tourStack
              .map((tour, index) => {
                ...
                const icon = isRoot ? '◆' : '◇';
                return `<div class="stackRow ${isRoot ? 'root' : 'tangent'} ${isActive ? 'active' : ''}"><button class="stackChip" data-tour-index="${index}">${icon} #${level} ${escapeHtml(tour.topic)}</button><button class="stackDiscard" data-discard-tour-index="${index}" title="Discard tour">×</button></div>`;
              })
              .join('')}
          </div>
        </div>`
